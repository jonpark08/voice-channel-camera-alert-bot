...
async def on_voice_state_update(member, before, after):
    ...
    def sendMsg(streamType, member, channel) -> discord.Embed:
        ...
        embed.set_footer(
            text="Voice Channel Camera Alert Bot",
    ...
    if after.channel and after.channel.id == int(channel_id['voice']):
        ...
        if after.self_stream:
            ...
            message = f'@everyone {member} has started streaming in {after.channel.name}'
            await alert_channel.send(message, delete_after=int(config['TIME']['delete_timeout']))
            logging.info(message)
            embedInstance = sendMsg("🖥️  『 영상", member, after.channel)
        ...
        if after.self_video:
            ...
            message = f'@everyone {member} has started camera in {after.channel.name}'
            await alert_channel.send(message, delete_after=int(config['TIME']['delete_timeout']))
            logging.info(message)
            embedInstance = sendMsg("📷  『 카메라", member, after.channel)
            await alert_channel.send("", embed=embedInstance, delete_after=int(config['TIME']['delete_timeout']))
